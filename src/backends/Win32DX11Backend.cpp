...
Backend::Backend(int width, int height, std::string_view title)
{
    ...
    g_hwnd = ::CreateWindow(g_wc.lpszClassName, title.data(), WS_OVERLAPPEDWINDOW, 100, 100, width, height, NULL, NULL, g_wc.hInstance, NULL);
    ...
    switch (msg)
    {
        ...
        case WM_DROPFILES:
        {
            ...
            for (UINT i = 0; i < count; ++i)
            {
                ...
                auto buffer = std::make_unique<WCHAR[]>(length + 1 * sizeof(WCHAR));

                DragQueryFileW(drop, i, buffer.get(), length + 1);

                try
                {
                    System::DroppedFiles.emplace_back(std::filesystem::canonical(buffer.get()));
                }
                catch (...)
                {
                }
