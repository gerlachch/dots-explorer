...
static ImGui_ImplVulkanH_Window* g_wd = nullptr;
...
GlfwBackend::GlfwBackend(int width, int height, std::string_view title)
{
    ...
    {
        throw std::logic_error{ "only one instance of GlfwVulkanBackend can be created at a given time" };
    }
    ...
    {
        throw std::runtime_error{ "GLFW does not support Vulkan not supported on this platform" };
    }
